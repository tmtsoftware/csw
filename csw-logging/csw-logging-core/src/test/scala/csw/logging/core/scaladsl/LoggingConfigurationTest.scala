...
import csw.logging.core.appenders.{FileAppender, StdOutAppender}
...
import csw.logging.core.internal.LoggingLevels.{DEBUG, INFO, TRACE}
import csw.logging.core.utils.FileUtils
  ...
  test("should log messages in the file with standard headers") {
    val config =
      ConfigFactory
        .parseString(s"""
                        |csw-logging {
                        | appenders = ["csw.logging.core.appenders.FileAppender$$"]
                        | appender-config {
                        |   file {
                        |     fullHeaders = true
  ...
  test("should log messages on the console with standard headers based on the log level configured in the config") {
    val config =
      ConfigFactory
        .parseString("""
                        |csw-logging {
                        | appenders = ["csw.logging.core.appenders.StdOutAppender$"]
                        | appender-config {
                        |   stdout {
                        |     fullHeaders = true
                        |     logLevelLimit = info
