...
class MailboxRequirementTest extends FunSuite with Matchers with BeforeAndAfterEach {
  ...
  test("should get all messages if msg count is under the capacity defined for mailbox") {
    val actorSystem        = ActorSystem(SpawnProtocol.behavior, "test", configWithCapacity(capacity = defaultCapacity))
    lazy val loggingSystem = new LoggingSystem("logging", "version", hostName, actorSystem)

    loggingSystem.setAppenders(List(testAppender))

    val irisActorRef: ActorRef = actorSystem.toUntyped.actorOf(IRIS.props(IRIS.COMPONENT_NAME), name = "IRIS-Supervisor-Actor")
  ...
  test("should get no messages if mailbox capacity is zero") {
    val actorSystem        = ActorSystem(SpawnProtocol.behavior, "test", configWithCapacity(capacity = zeroCapacity))
    lazy val loggingSystem = new LoggingSystem("logging", "version", hostName, actorSystem)
