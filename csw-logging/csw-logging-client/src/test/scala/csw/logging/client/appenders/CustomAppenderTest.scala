...
class CustomAppenderTest extends AnyFunSuite with Matchers {
  ...
  test("should be able to add and configure a custom appender using an object extending from CustomAppenderBuilder | DEOPSCSW-272") {
  ...
  test("should be able to add and configure a custom appender using a class extending from CustomAppenderBuilder showing config changes | DEOPSCSW-272") {
