...
import csw.services.logging.scaladsl.{LoggerFactory, LoggerImpl}
...
private[logging] abstract class JBaseLoggerFactory(maybeComponentName: Option[String]) {
  def getLogger[T](ctx: ActorContext[T], klass: Class[_]): ILogger = {
    val logger = new LoggerImpl(maybeComponentName, Some(actorPath(ctx.getSelf.toUntyped)))
    new JLogger(logger, klass)
  }

  def getLogger(ctx: actor.ActorContext, klass: Class[_]): ILogger = {
    val logger = new LoggerImpl(maybeComponentName, Some(actorPath(ctx.self)))
    new JLogger(logger, klass)
  }

  def getLogger(klass: Class[_]): ILogger = {
    val logger = new LoggerImpl(maybeComponentName, None)
    new JLogger(logger, klass)
  }
