...
docker exec -itd Assembly bash -c 'ifconfig && cd /source/csw && ./target/universal/stage/bin/assembly-app -DclusterPort=3552'

printf "${PURPLE}------ Waiting for 10 seconds to let Assembly gets started ------${NC}\n"
sleep 10
printf "${YELLOW} Executing test in network : tmt_net_2 ${NC}\n"
docker exec Test-App bash -c 'ifconfig && cd /source/csw && ./target/universal/stage/bin/test-multiple-nic-app -DclusterSeeds=172.17.0.2:3552'
exit_code=$?
