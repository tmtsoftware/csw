...
lazy val `csw-framework` = project
  .dependsOn(
    ...
    `csw-location`      % "test->multi-jvm;multi-jvm->multi-jvm",
...
lazy val `csw-event-client` = project
  .dependsOn(
    ...
    `romaine`,
    `csw-location-api`,
    `csw-location` % "test->compile;test->multi-jvm;multi-jvm->multi-jvm",
    `csw-commons`  % "test->test"
  )
  .enablePlugins(PublishBintray, AutoMultiJvm, GenJavadocPlugin, MaybeCoverage)
...
lazy val `csw-alarm-client` = project
  .dependsOn(
    ...
    `csw-commons`  % "test->test",
    `csw-location` % "test->compile;test->multi-jvm"
  )
  .enablePlugins(PublishBintray, GenJavadocPlugin, MaybeCoverage)
...
lazy val `csw-benchmark` = project
  .dependsOn(
    ...
    `csw-params-jvm`,
    `csw-command`,
    `csw-location` % "compile->multi-jvm",
