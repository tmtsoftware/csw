...
object CommandExecutionResponse {
  final case class InProgress(runId: RunId, message: String = "")
      extends CommandExecutionResponse(CommandResultType.Intermediate)
  final case class Initialized(runId: RunId) extends CommandExecutionResponse(CommandResultType.Intermediate)

  case class CompletedWithResult(runId: RunId, result: Result)
      extends CommandExecutionResponse(CommandResultType.Positive)
  case class Completed(runId: RunId) extends CommandExecutionResponse(CommandResultType.Positive)
  case class BehaviorChanged[T](runId: RunId, ref: ActorRef[T])
      extends CommandExecutionResponse(CommandResultType.Positive)

  case class NoLongerValid(runId: RunId, issue: CommandIssue)
      extends CommandExecutionResponse(CommandResultType.Negative)
