...
object CommandExecutionResponse {
  ...
  final case class CompletedWithResult(runId: RunId, result: Result)
      extends CommandExecutionResponse
   ...
   */
  final case class Completed(runId: RunId) extends CommandExecutionResponse with CommandFinalResponse
   ...
   */
  final case class Error(runId: RunId, message: String) extends CommandExecutionResponse with CommandNegativeResponse

  /**
   * The command was aborted
   * Aborted means that the command/actions were stopped immediately.
   */
  final case class Aborted(runId: RunId) extends CommandExecutionResponse with CommandNegativeResponse

  /**
   * The command was cancelled
   * Cancelled means the command/actions were stopped at the next convenient place. This is usually appropriate for
   */
  final case class Cancelled(runId: RunId) extends CommandExecutionResponse with CommandNegativeResponse

  final case class BehaviorChanged[T](runId: RunId, ref: ActorRef[T])
      extends CommandExecutionResponse
      with CommandFinalResponse

  final case class NotStarted(runId: RunId) extends CommandExecutionResponse with CommandIntermediateResponse

  final case class CommandNotAvailable(runId: RunId) extends CommandExecutionResponse
}
