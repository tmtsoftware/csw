...
class SerializationTest extends FunSuite {
  ...
  val sc1: Setup = Setup(obsId, Prefix("tcs.pos")).madd(
    ra.set("12:32:11"),
    dec.set("30:22:22"),
    epoch.set(1950.0),
    test.set(1),
  ...
  val sc2: Setup = Setup(obsId, Prefix("wfos.blue"))
    .add(disperser.set("gr243"))
    .add(filter1.set("GG433"))

  val ob1: Observe = Observe(obsId, Prefix("wfos.blue.camera"))
    .add(exposureTime.set(22.3)) // .sec,
    .add(repeats.set(3))

  val wc1: Wait = Wait(obsId, Prefix("wfos.blue.camera"))
