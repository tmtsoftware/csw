...
## Running latest master of config-cli on developer machine
...
*  @ref:[csw-location-agent](./../apps/cswlocationagent.md) application is running, which has started the Config Server and registered it to the Location Service.
...
1.  To run the latest master on dev machine the command `sbt run` can be used.
    ```bash
      // run config-cli using sbt with arguments
      sbt "csw-config-cli/run login"
    ```


2. Alternatively the command `sbt publishLocal` followed by `cs launch config-cli:0.1.0-SNAPSHOT` can be used.
   Command line parameters can also be passed while launching SNAPSHOT version using coursier.
    ```bash
      // run config-cli using coursier
      cs launch config-cli:0.1.0-SNAPSHOT -- login
    ```  
