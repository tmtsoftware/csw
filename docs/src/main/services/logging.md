 ...
 * Logs generated by components will be by default stored at `$HOME/tmt/logs`. In case $HOME is not set as environment variable,
then they will be stored at `/tmp/tmt/logs`. When logs are stored in /tmp folder they will not be persisted in case of machine
reboot. Hence, for production, it is recommended to have $HOME set as environment variable.

* `csw-logging.appender-config.file.logPath` from `logging.conf` will treated as subdirectory under base directory ($HOME/tmt/logs or
