...
class RedisPSubscribeApi[K, V](redisReactiveCommands: RedisPubSubReactiveCommands[K, V])(implicit ec: ExecutionContext)
  ...
  def subscribe(keys: List[K]): Future[Done] = redisReactiveCommands.psubscribe(keys*).toFuture.asScala.map(_ => Done)
  ...
  def unsubscribe(keys: List[K]): Future[Done] = redisReactiveCommands.punsubscribe(keys*).toFuture.asScala.map(_ => Done)
