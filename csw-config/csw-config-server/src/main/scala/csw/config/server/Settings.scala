...
class Settings(config: Config) {
  ...
  private val `pekko.http.server.parsing` = config.getConfig("pekko.http.server.parsing")
  ...
  val `max-content-length`: String     = `pekko.http.server.parsing`.getString("max-content-length")
