...
class KafkaPublisher(producerSettings: ProducerSettings[String, Array[Byte]])(implicit ec: ExecutionContext, mat: Materializer)
  ...
  override def publish(event: Event): Future[Done] = {
    ...
    try {
      ...
      case NonFatal(ex) ⇒ promisedDone.failure(PublishFailedException(event))
    }
    promisedDone.future
  }
  ...
  private def publishWithRecovery[Mat](source: Source[Event, Mat], maybeOnError: Option[Event ⇒ Unit]): Mat =
    source
      .mapAsync(100) {
        maybeOnError match {
          case Some(onError) ⇒ publish(_).recover { case _ @PublishFailedException(event) ⇒ onError(event) }
          case None          ⇒ publish
        }
      }
      .mapError {
