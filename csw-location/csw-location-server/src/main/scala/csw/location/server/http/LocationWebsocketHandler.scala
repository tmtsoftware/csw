...
class LocationWebsocketHandler(locationServiceE: LocationServiceE, val encoding: Encoding[_])
  ...
  override def handle(request: LocationWebsocketMessage): Source[Message, NotUsed] = request match {
    case Track(connection) => stream(locationServiceE.track(connection))
  }
}
