...
import csw.location.impl.commons.CswCluster
import csw.location.impl.internal.{LocationServiceFactory, ServerWiring}
  ...
  test("should detect re-registering of new location for a connection that has crashed/gone away") {
    ...
    runOn(member1) {
      ...
      val freshLocationService = mode match {
        case "http" =>
          Try(ServerWiring.make(newTypedSystem, "csw-location-server").locationHttpService.start().await) match {
