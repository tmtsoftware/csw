...
final case class AkkaLocation private (connection: AkkaConnection, uri: URI, actorRef: ActorRef) extends Location {
...
final case class TcpLocation private (connection: TcpConnection, uri: URI) extends Location {
  def this(connection: TcpConnection, hostname: String, port: Int) = {
    this(connection, new URI(s"tcp://$hostname:$port"))
  }
}

final case class HttpLocation private (connection: HttpConnection, uri: URI) extends Location {
