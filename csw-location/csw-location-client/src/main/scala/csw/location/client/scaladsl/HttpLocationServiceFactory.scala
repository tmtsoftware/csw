...
object HttpLocationServiceFactory {
   ...
   * */
  def makeLocalClient(implicit actorSystem: ActorSystem[SpawnProtocol], mat: Materializer): LocationService = make("localhost")

  /**
   * Use this factory method to create http location client when location server ip is known.
   * HTTP Location server runs on port 7654.
   * */
  private[csw] def make(serverIp: String)(implicit actorSystem: ActorSystem[SpawnProtocol], mat: Materializer): LocationService =
