...
class ComponentTypeTest extends AnyFunSuite with Matchers with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  test(
    "ComponentType should be any one of this types : 'container', 'hcd', 'assembly', 'sequence', 'sequence_component', 'sequence_manager' and 'service' | DEOPSCSW-14"
  ) {
    val expectedComponentTypeValues =
      Set("container", "hcd", "assembly", "service", "machine", "sequencer", "sequence_component", "sequence_manager")
