...
class IdTest extends AnyFunSuite with Matchers {
  ...
  test("should able to create setup commands having unique runId | DEOPSCSW-205") {
    ...
    1 to 10 foreach { _ =>
      val setup = Setup(Prefix("wfos.move.window"), CommandName("test"), Some(ObsId("2020A-P001-O123")), Set.empty)
  ...
  test("should able to create observe commands having unique runId | DEOPSCSW-205") {
    ...
    1 to 10 foreach { _ =>
      val observe = Observe(Prefix("wfos.move.window"), CommandName("test"), Some(ObsId("2020A-P001-O123")), Set.empty)
  ...
  test("should able to create wait commands having unique runId | DEOPSCSW-205") {
    ...
    1 to 10 foreach { _ =>
      val wait = Wait(Prefix("wfos.move.window"), CommandName("test"), Some(ObsId("2020A-P001-O123")), Set.empty)
