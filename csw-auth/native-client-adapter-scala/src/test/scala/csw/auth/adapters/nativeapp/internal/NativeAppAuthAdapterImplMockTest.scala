...
class NativeAppAuthAdapterImplMockTest extends FunSuite with MockitoSugar with Matchers {
  ...
  test("getAccessToken") {
    ...
    authService.getAccessToken() shouldBe TokenVerifier().verifyAndDecode(token).toOption
  ...
  test("refreshing token") {
    ...
    authService.getAccessToken(200.seconds) shouldBe TokenVerifier().verifyAndDecode(refreshedToken).toOption
