...
object AlarmStatus {
  ...
  def apply(): AlarmStatus = AlarmStatus(Unacknowledged, Disconnected, Unshelved, None)
  //todo: make acknowledged as default value since we don't want users to have to ack disconnected severity
}
