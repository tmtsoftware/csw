...
# Usage is:
#
#  csw-services.sh PATH_TO_TRACKLOCATION   - to start redis and register it for the event, alarm and telemetry services
#
...
# Run a command and register services using trackLocation
$PATH_TO_TRACKLOCATION_APP/tracklocation --name "$SERVICES" --port $PORT --command "$COMMAND"

# Other usages
# $PATH_TO_TRACKLOCATION_APP --name "$SERVICES" --command "$COMMAND"
# $PATH_TO_TRACKLOCATION_APP --name "$SERVICES" --port $PORT "
# $PATH_TO_TRACKLOCATION_APP --name "$SERVICES"