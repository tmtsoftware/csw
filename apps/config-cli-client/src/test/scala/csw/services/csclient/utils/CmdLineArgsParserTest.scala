...
class CmdLineArgsParserTest
  ...
  test("test create with all applicable sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("create", Some(Paths.get(repositoryFilePath)), Some(Paths.get(inputFilePath)), None, None, Int.MaxValue, true, comment))
  }

  test("test create with bare minimum sub-options") {
    val argv = Array("create", repositoryFilePath, "-i", inputFilePath)
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("create", Some(Paths.get(repositoryFilePath)), Some(Paths.get(inputFilePath)), None, None, Int.MaxValue, false, ""))
  }
  ...
  test("test update with all applicable sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("update", Some(Paths.get(repositoryFilePath)), Some(Paths.get(inputFilePath)), None, None, Int.MaxValue, false, comment))
  }

  test("test update with bare minimum sub-options") {
    val argv = Array("update", repositoryFilePath, "-i", inputFilePath)
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("update", Some(Paths.get(repositoryFilePath)), Some(Paths.get(inputFilePath)), None, None, Int.MaxValue, false, ""))
  }
  ...
  test("test get with all applicable sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("get", Some(Paths.get(repositoryFilePath)), None, Some(Paths.get(outputFilePath)), Some(id), Int.MaxValue, false, ""))
  }

  test("test get with bare minimum sub-options") {
    val argv = Array("get", repositoryFilePath, "-o", outputFilePath)
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("get", Some(Paths.get(repositoryFilePath)), None, Some(Paths.get(outputFilePath)), None, Int.MaxValue, false, ""))
  }
  ...
  test("test exists with sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("exists", Some(Paths.get(repositoryFilePath)), None, None, None, Int.MaxValue, false, ""))
  }
  ...
  test("test delete with sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("delete", Some(Paths.get(repositoryFilePath)), None, None, None, Int.MaxValue, false, ""))
  }

  test("test list") {
    val argv = Array("list")
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("list", None, None, None, None, Int.MaxValue, false, ""))
  }
  ...
  test("test history with sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("history", Some(Paths.get(repositoryFilePath)), None, None, None, maxFileVersions, false, ""))
  }
  ...
  test("test setDefault with all sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("setDefault", Some(Paths.get(repositoryFilePath)), None, None, Some(id), Int.MaxValue, false, ""))
  }

  test("test setDefault with bare minimum sub-options") {
    val argv = Array("setDefault", repositoryFilePath)
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("setDefault", Some(Paths.get(repositoryFilePath)), None, None, None, Int.MaxValue, false, ""))
  }
  ...
  test("test resetDefault with sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("resetDefault", Some(Paths.get(repositoryFilePath)), None, None, None, Int.MaxValue, false, ""))
  }
  ...
  test("test getDefault with sub-options") {
    ...
    val x: Option[Options] = CmdLineArgsParser.parser.parse(argv, Options())
    x should contain (new Options("getDefault", Some(Paths.get(repositoryFilePath)), None, Some(Paths.get(outputFilePath)), None, Int.MaxValue, false, ""))
  }
}