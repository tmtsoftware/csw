...
import csw.command.client.messages.{ComponentMessage, ContainerMessage, SequencerMsg}
...
object AkkaLocationExt {
  implicit class RichAkkaLocation(val akkaLocation: AkkaLocation) {
    ...
    private[command] def sequencerRef(implicit actorSystem: ActorSystem[_]): ActorRef[SequencerMsg] = typedRef[SequencerMsg]
