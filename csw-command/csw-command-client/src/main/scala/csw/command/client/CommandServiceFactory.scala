...
object CommandServiceFactory {
  ...
  private[csw] def make(component: ActorRef[ComponentMessage])(implicit actorSystem: ActorSystem[?]): CommandService =
   ...
   */
  def jMake(componentLocation: Location, actorSystem: ActorSystem[?]): ICommandService =
    new JCommandServiceImpl(make(componentLocation)(actorSystem))

  private def httpClient(httpLocation: HttpLocation)(implicit system: ActorSystem[?]) = {
