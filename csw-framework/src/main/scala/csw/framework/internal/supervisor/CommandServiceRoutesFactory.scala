...
import csw.command.client.handlers.{CommandServiceRequestHandler, CommandServiceStreamingRequestHandlers}
...
object CommandServiceRoutesFactory {
  ...
  def createRoutes(component: ActorRef[ComponentMessage])(implicit actorSystem: ActorSystem[_]): Route = {
    ...
    val websocketHandlers  = new CommandServiceStreamingRequestHandlers(commandService)
