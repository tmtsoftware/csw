...
abstract class ComponentBehaviorFactory {
   ...
   */
  protected def handlers(ctx: ActorContext[TopLevelActorMessage], cswServices: CswServices): ComponentHandlers
   ...
   */
  private[framework] def make(supervisor: ActorRef[FromComponentLifecycleMessage], cswServices: CswServices): Behavior[Nothing] =
    Behaviors
      .setup[TopLevelActorMessage] { ctx ⇒
        new ComponentBehavior(ctx, supervisor, handlers(ctx, cswServices), cswServices)
      }
      .narrow
}
