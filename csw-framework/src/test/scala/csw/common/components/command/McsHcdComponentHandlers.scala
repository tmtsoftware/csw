...
class McsHcdComponentHandlers(
  ...
  override def onDomainMsg(msg: ComponentDomainMessage): Unit = msg match {
    case LongCommandCompleted(runId, commandResponse)   => commandResponseManager ! AddOrUpdateCommand(runId, commandResponse)
    case MediumCommandCompleted(runId, commandResponse) => commandResponseManager ! AddOrUpdateCommand(runId, commandResponse)
    case ShortCommandCompleted(runId, commandResponse)  => commandResponseManager ! AddOrUpdateCommand(runId, commandResponse)
  }
  ...
  override def onSubmit(controlCommand: ControlCommand, replyTo: ActorRef[CommandResponse]): Unit = controlCommand.prefix match {
    case `longRunningCmdPrefix` =>
      ctx.schedule(5.seconds, ctx.self, LongCommandCompleted(controlCommand.runId, Completed(controlCommand.runId)))
    case `mediumRunningCmdPrefix` =>
      ctx.schedule(3.seconds, ctx.self, MediumCommandCompleted(controlCommand.runId, Completed(controlCommand.runId)))
    case `shortRunningCmdPrefix` =>
      ctx.schedule(1.seconds, ctx.self, ShortCommandCompleted(controlCommand.runId, Completed(controlCommand.runId)))
  }
